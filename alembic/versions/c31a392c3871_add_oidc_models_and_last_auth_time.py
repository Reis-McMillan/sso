...
def upgrade() -> None:
    ...
    columns = [c['name'] for c in inspector.get_columns('identity')]
    if 'last_auth_time' not in columns:
        op.add_column(
            'identity',
    ...
    if not inspector.has_table('oauth_client'):
        op.create_table(
            'oauth_client',
    ...
    if not inspector.has_table('authorization_code'):
        op.create_table(
            'authorization_code',
            sa.Column('id', sa.Integer, primary_key=True),
    ...
    if not inspector.has_table('refresh_token'):
        op.create_table(
            'refresh_token',
            sa.Column('id', sa.Integer, primary_key=True),
    ...
    if not inspector.has_table('consent'):
        op.create_table(
            'consent',
            sa.Column('id', sa.Integer, primary_key=True),
            sa.Column('identity_email', sa.String, nullable=False),
    ...
    if not inspector.has_table('oauth2_session'):
        op.create_table(
            'oauth2_session',
            sa.Column('id', sa.Integer, primary_key=True),
